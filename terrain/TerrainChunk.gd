class_name TerrainChunk
extends Node3D

var height: float = 1.0
var mesh_instance: MeshInstance3D
var noise: FastNoiseLite
var x: int
var z: int
var chunk_size: float

func _init(noise: FastNoiseLite, x: int, z: int, chunk_size: float) -> void:
	self.noise = noise
	self.x = x
	self.z = z
	self.chunk_size = chunk_size

func _ready() -> void:
	generate_chunk()

func generate_chunk() -> void:
	var plane_mesh: PlaneMesh = PlaneMesh.new()
	plane_mesh.size = Vector2(chunk_size, chunk_size)
	plane_mesh.subdivide_depth = chunk_size * 0.5
	plane_mesh.subdivide_width = chunk_size * 0.5

	#add material here
	plane_mesh.material = load("res://terrain/material/moon_rock.tres")
	var surface_tool: SurfaceTool = SurfaceTool.new()
	var data_tool: MeshDataTool = MeshDataTool.new()
	surface_tool.create_from(plane_mesh, 0)

	var array_plane: ArrayMesh = surface_tool.commit()
	var error = data_tool.create_from_surface(array_plane, 0)

	for i in range(data_tool.get_vertex_count()):
		var vertex: Vector3 = data_tool.get_vertex(i)
		vertex.y = noise.get_noise_3d(vertex.x + x, vertex.y, vertex.z + z) * height

		data_tool.set_vertex(i, vertex)

	#for s in range(array_plane.get_surface_count()):
	array_plane.clear_surfaces()

	data_tool.commit_to_surface(array_plane)
	surface_tool.begin(Mesh.PRIMITIVE_TRIANGLES)
	surface_tool.create_from(array_plane, 0)
	surface_tool.generate_normals()

	mesh_instance= MeshInstance3D.new()
	mesh_instance.mesh = surface_tool.commit()
	mesh_instance.create_trimesh_collision()
	mesh_instance.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_ON
	add_child(mesh_instance)
